	...
	try:
		...
		if backup_key:
			...
			if 'Contents' in results:
				...
				old_content = s3_resource_.Object(bucket, key).get()['Body'].read().decode('utf-8')
				s3_client_.put_object(Body=old_content, Bucket=bucket, Key=rename_file(key, backup_key, backup_strategy))
